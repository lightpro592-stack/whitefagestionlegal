...
2. Crée un nouveau tableur, par exemple `Mysteria FA`.
...
5. Crée un compte de service, par exemple `mysteria-fa-sheets`.
...
Le token est visible dans `localStorage`, clé `mysteria-fa-session`, après connexion admin.
...
Pour `GOOGLE_PRIVATE_KEY`, colle la cl� priv�e compl�te avec les `\n`. Apr�s modification des variables, lance un nouveau `Redeploy`.
